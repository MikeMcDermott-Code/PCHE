...
#include "myvariableHeatTransfer.H"
...
namespace fv
{
    defineTypeNameAndDebug(myvariableHeatTransfer, 0);
    addToRunTimeSelectionTable(option, myvariableHeatTransfer, dictionary);
}
}
...
Foam::fv::myvariableHeatTransfer::myvariableHeatTransfer
(
    const word& name,
...
:
    ...
    PorosityvFluidNbrName_(coeffs_.getOrDefault<word>("PorosityvFluidNbr", "PorosityvFluid")),    
    a_(0),
    b_(0),
    c_(0),
    ds_(0),
...
void Foam::fv::myvariableHeatTransfer::calculateHtc()
{
    ...
    const auto& PorosityvFluidNbr = nbrMesh.lookupObject<volScalarField>(PorosityvFluidNbrName_);
    
//    tmp<volScalarField> ReNbr(mag(UNbr)*ds_*nbrThermo.rho()/nbrTurb.mut());
    tmp<volScalarField> ReNbr(mag(UNbr/PorosityvFluidNbr)*ds_*nbrThermo.rho()/nbrThermo.mu());
    
    tmp<volScalarField> NuNbr((4/ds_)*PorosityvFluidNbr*a_*pow(ReNbr, b_)*pow(Pr_, c_));

//   scalarField htcNbr(NuNbr*nbrTurb.kappaEff()/ds_);
    scalarField htcNbr(NuNbr*nbrThermo.kappa()/ds_);
...
bool Foam::fv::myvariableHeatTransfer::read(const dictionary& dict)
{
    if (interRegionHeatTransferModel::read(dict))
    {
        coeffs_.readIfPresent("UNbr", UNbrName_);
        coeffs_.readIfPresent("PorosityvFluidNbr", PorosityvFluidNbrName_);
